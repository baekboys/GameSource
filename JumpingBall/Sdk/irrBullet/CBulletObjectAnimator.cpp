...
IMesh* CBulletObjectAnimator::GetMeshFromGeom(ISceneManager* pSceneManager,
  CBulletObjectAnimatorGeometry* pGeom)
{
  IMesh* mesh = pGeom->geometryUnion.mesh.irrMesh;
  if (mesh == NULL)
  {
  ...
  btStridingMeshInterface*& triangleMesh)
{
	...
	case CBPAGT_CAPSULE:
		{
			collisionShape = new btCapsuleShape(pGeom->geometryUnion.Capsule.radius,pGeom->geometryUnion.Capsule.hight);
		}
		break;
	case CBPAGT_SPHERE:
		{
			collisionShape = new btSphereShape(pGeom->geometryUnion.sphere.radius);
		}
		break;

	case CBPAGT_BOX:
		{
			collisionShape = new btBoxShape(btVector3(pGeom->geometryUnion.box.X, pGeom->geometryUnion.box.Y, pGeom->geometryUnion.box.Z));
		} 
		break;
	case CBPAGT_CYLINDER:
		{
			collisionShape = new btCylinderShape(btVector3(pGeom->geometryUnion.box.X, pGeom->geometryUnion.box.Y, pGeom->geometryUnion.box.Z));
		}
		break;
	...
	case CBPAGT_CONCAVE_LOD_TERRAIN_MESH:
		{
			...
			//메쉬컨버팅
			{
				...
				pTerrain->getMeshBufferForLOD(smb,pGeom->geometryUnion.terrain_mesh.meshLod);				
...
void CBulletObjectAnimator::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options/*=0*/)
{
  ...
  switch (geometry.type)
  {
    case CBPAGT_SPHERE:
    {
      out->addFloat("radius", geometry.geometryUnion.sphere.radius);
    } break;
    case CBPAGT_BOX:
    {
      out->addVector3d("halfExtents", core::vector3df(geometry.geometryUnion.box.X, geometry.geometryUnion.box.Y, geometry.geometryUnion.box.Z));
    } break;
    case CBPAGT_CONVEX_MESH:
    case CBPAGT_CONCAVE_MESH:
    case CBPAGT_CONCAVE_GIMPACT_MESH:
    {
      if (geometry.geometryUnion.mesh.irrMesh != NULL)
      {
...
void CBulletObjectAnimator::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options/*=0*/)
{
  ...
  switch (geometry.type)
  {
    case CBPAGT_SPHERE:
    {
      geometry.geometryUnion.sphere.radius = in->getAttributeAsFloat("radius");
    } break;
    case CBPAGT_BOX:
    {
      ip = in->getAttributeAsVector3d("halfExtents");
      geometry.geometryUnion.box.X = ip.X;
      geometry.geometryUnion.box.Y = ip.Y;
      geometry.geometryUnion.box.Z = ip.Z;
    } break;
    case CBPAGT_CONVEX_MESH:
    case CBPAGT_CONCAVE_MESH:
    case CBPAGT_CONCAVE_GIMPACT_MESH:
